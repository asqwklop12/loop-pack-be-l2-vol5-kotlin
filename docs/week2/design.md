...
이번 주의 과제는 기능을 늘리는 것이 아니라 **누가 무엇을 책임지는지 정하고, 그 책임이
코드와 검사로 지켜지게 만드는 것**이다. 그래서 세 가지를 축으로 삼았다.

| 축 | 무엇을 했나 |
| --- | --- |
| **책임** | 상태와 규칙을 한 객체에 모으고, 순서 조율과 결과 조합은 application 에 두었다 |
| **의존** | `domain` 으로만 화살표가 들어오게 하고 ArchUnit 으로 고정했다 |
| **검증** | 중요한 규칙은 TDD 로, 반복해서 지킬 구조는 ArchUnit·ktlint 로 확인했다 |

설계 판단은 대부분 흐름도에서 나왔다. 유스케이스 22개를 먼저 그리고 거기서 단계·분기·실패를
읽어 구현했다. **흐름도가 기준**이고, 코드와 어긋나면 코드를 고쳤다.

범위에서 뺀 것은 쿠폰·외부 결제(PG)·배송·사용자 도메인이다. 몰라서 뺀 것이 아니라
지금 검증할 대상이 없어서 뺐고, 각각 다시 볼 조건을 8절에 적었다.
