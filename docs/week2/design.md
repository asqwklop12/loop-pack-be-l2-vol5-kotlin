...
**같이 지켜야 할 규칙이 있으면 한 애그리거트로 묶고 객체로 참조한다. 없으면 나누고 id 로
참조한다.** 애그리거트에 걸친 조건은 도메인 서비스가 조회해서 판단한다.

| 애그리거트 | 안에 있는 것 | 묶인 이유 |
| --- | --- | --- |
| `Order` | `OrderLine` | 합계가 라인 전체를 봐야 성립한다. 라인만 따로 바꾸면 규칙이 깨진다 |
| `Brand` | — | 단독 |
| `Product` | — | 단독 |
| `Like` | — | 단독 |
| `PointBalance` | — | 단독 |

객체 참조는 `Order → OrderLine` 하나뿐이고 나머지는 전부 id 다.
...
`Brand` 와 `Product` 를 묶지 않은 이유는 셋이다. 브랜드 하나에 상품이 수천 개일 수 있어
이름 한 글자를 고치는 데 상품 전부를 읽게 되고, 상품은 목록·검색·주문에서 직접 조회하며,
브랜드와 무관하게 만들어지고 수정된다.

브랜드 삭제 조건("삭제되지 않은 상품이 0개")은 **항상 성립해야 하는 불변식이 아니라
그 순간에만 확인하는 선행 조건**이다. 그래서 묶지 않고 `BrandService.delete` 가 상품 수를
조회해 판단한다.
...
| 어디서 | 무엇을 조회해 판단하나 |
| --- | --- |
| `BrandService.delete` | 그 브랜드의 삭제되지 않은 상품 수 |
| `ProductService.create` | 브랜드의 존재·삭제 여부 |
| `ProductService.delete` | 재고, 그리고 좋아요 관계를 함께 지운다 |
| `LikeService.like` | 상품의 존재·삭제 여부 |
| `OrderService` | 상품과 포인트 잔액 |
